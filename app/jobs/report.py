...
class Report:
    ...
    def _fmt_date(timestamp):
        if "." in timestamp:
            return (
                timestamp[0 : timestamp.find(".")]
                + timestamp[timestamp.find(" ", timestamp.find(".")) :]
            )
        else:
            return timestamp

    @staticmethod
    def _get_uptime_downtime(
        start, end, business_ranges, uptime_ranges, downtime_ranges
    ):
        ...
        for business_range in business_ranges:
            if business_range[0] < end and business_range[1] > start:
                business_time += min(business_range[1], end) - max(
                    business_range[0], start
                )
    ...
    def _get_business_ranges(
        self, store, current_timestamp, store_hours, store_time_zones
    ):
        # Get store timezone
        store_time_zone = DEFAULT_TIMEZONE
        if store_time_zones["store_id"].isin([store]).any():
            store_time_zone = store_time_zones.loc[
                store_time_zones["store_id"].isin([store]), "timezone_str"
            ].values[0]
        ...
        timestamp_in_store_tz = current_timestamp.astimezone(
            pytz.timezone(store_time_zone)
        )
        date_in_store_tz = timestamp_in_store_tz.replace(
        ...
        for i in range(8):
            ...
            if (this_store_hours["day"].isin([day])).any():
                business_24x7 = False
                business_hours = this_store_hours.loc[
                    this_store_hours["day"].isin([day])
                ]
                business_start_time = datetime.datetime.strptime(
                    business_hours.loc[:, "start_time_local"].values[0], "%H:%M:%S"
                )
                business_end_time = datetime.datetime.strptime(
                    business_hours.loc[:, "end_time_local"].values[0], "%H:%M:%S"
                )
            ...
            business_hours_timestamp_ranges.insert(
                0,
                [
                    business_start_timestamp.astimezone(pytz.utc),
                    business_end_timestamp.astimezone(pytz.utc),
                ],
            )
        ...
        business_hours_timestamp_ranges = self._merge_ranges(
            business_hours_timestamp_ranges
        )
    ...
    def _get_ud_range(this_store_status_polls, business_hours_timestamp_ranges):
        ...
        for business_hours_timestamp_range in business_hours_timestamp_ranges:
            polls_within_range = this_store_status_polls.loc[
                (
                    this_store_status_polls["timestamp_utc"]
                    >= business_hours_timestamp_range[0]
                )
                & (
                    this_store_status_polls["timestamp_utc"]
                    <= business_hours_timestamp_range[1]
                )
            ]
            ...
            lower_timestamp_index = polls_within_range.columns.get_loc(
                "lower_timestamp"
            )
            upper_timestamp_index = polls_within_range.columns.get_loc(
                "upper_timestamp"
            )
            ...
            for i in range(len(polls_within_range)):
                ...
                if i == 0:
                    polls_within_range.iat[
                        i, lower_timestamp_index
                    ] = business_hours_timestamp_range[0]
                if i == len(polls_within_range) - 1:
                    polls_within_range.iat[
                        i, upper_timestamp_index
                    ] = business_hours_timestamp_range[1]
                if i > 0:
                    previous_poll_timestamp = polls_within_range.iat[
                        i - 1, timestamp_utc_index
                    ]
                    polls_within_range.iat[i, lower_timestamp_index] = (
                        previous_poll_timestamp
                        + (present_poll_timestamp - previous_poll_timestamp) / 2
                    )
                if i < len(polls_within_range) - 1:
                    next_poll_timestamp = polls_within_range.iat[
                        i + 1, timestamp_utc_index
                    ]
                    polls_within_range.iat[i, upper_timestamp_index] = (
                        present_poll_timestamp
                        + (next_poll_timestamp - present_poll_timestamp) / 2
                    )
                polls_within_range.iat[i, lower_timestamp_index] = max(
                    polls_within_range.iat[i, lower_timestamp_index],
                    present_poll_timestamp - CHOSEN_MARGIN,
                )
                polls_within_range.iat[i, upper_timestamp_index] = min(
                    polls_within_range.iat[i, upper_timestamp_index],
                    present_poll_timestamp + CHOSEN_MARGIN,
                )
    ...
    def process(self, report_id: str):
        try:
            ...
            store_status["timestamp_utc"] = store_status["timestamp_utc"].map(
                self._fmt_date
            )
            store_status["timestamp_utc"] = pd.to_datetime(
                store_status["timestamp_utc"], format="%Y-%m-%d %H:%M:%S %Z"
            )
            ...
            polls_for_each_store = store_status.loc[
                store_status["timestamp_utc"]
                >= (last_week_timestamp - datetime.timedelta(days=1))
            ].groupby("store_id")
            ...
            for store in store_IDs:
                ...
                business_hours_timestamp_ranges = self._get_business_ranges(
                    store, current_timestamp, store_hours, store_time_zones
                )
                ...
                uptime_ranges, downtime_ranges = self._get_ud_range(
                    this_store_status_polls, business_hours_timestamp_ranges
                )
