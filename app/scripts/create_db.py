...
with Postgres() as db_session:
    def load_data_from_csv(file_key: str, table):
        ...
        if not os.path.exists(os.path.join(LOG_PATH, file_name)):
            ...
            if not gsheet_id:
                raise RuntimeError("Could not download google sheet raw data. Update app.constants file")
            
            gdown.download(id=gsheet_id, output=os.path.join(LOG_PATH, file_name), quiet=False)
